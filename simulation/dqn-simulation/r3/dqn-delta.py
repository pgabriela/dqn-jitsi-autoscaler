...
def digitize(l):
    bins = [-1 * (10 ** x) for x in range(-1, -9, -1)]
    bins.append(0)
    bins.extend([1 * (10 ** x) for x in range(-8, 0, 1)])
    idx = np.digitize(l, bins, right=True)
    idx = int(np.clip(idx, 0, len(bins) - 1))
    return np.around(bins[idx], 10)
...
for i_episode in range(N_EPISODES):
    ...
    for i in range(len(conf_count_over_time)):
        ...
        if (i+1) % ACTION_COOLDOWN == 0:
            ...
            z_delta = state[2]
            l_delta = state[3]
            state = [[p_delta, j_delta, z_delta, digitize(l_delta), digitize(l1)]]
