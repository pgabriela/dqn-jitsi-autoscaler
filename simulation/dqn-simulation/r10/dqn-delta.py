...
def calc_reward(state, action):
    ...
    if loss_delta == 0:
        if conf_delta == 0 and curr_loss > 0:
            if jvb_num_delta == 1:
                reward = -1
            elif jvb_num_delta == -1:
                reward = 0
            else:
                reward = 1
        elif curr_loss == 0:
...
for i_episode in range(N_EPISODES):
    ...
    for i in range(len(conf_count_over_time)):
        ...
        if (i+1) % ACTION_COOLDOWN == 0:
            ...
            latest_losses = []
            curr_state = np.array([p1, j1, z1, l1, c1])
            ...
            l_delta = state[3]
            c_delta = state[4]
            state = [[p_delta, j_delta, z_delta, l_delta, l1, c_delta]]
