...
curr_time = time.time()
# Query InfluxDB
monitoring_data = influxdb_jcf.query("select conferences, participants, bridge_selector_bridge_count from jicofo_stats where time >= '2020-12-23T07:40:00Z' and time < '2020-12-23T09:03:00Z';")
monitoring_data2 = influxdb_jvb.query("select mean(jitter_aggregate) as jitter, mean(overall_loss) as loss, mean(rtt_aggregate) as rtt from jitsi_stats where time >= '2020-12-23T07:40:00Z' and time < '2020-12-23T09:03:00Z' group by time(1s) fill(0);")
...
plt.title('Resulted Total Conferences (Q-Learning)')
...
plt.title('Resulted Overall Jitters (Q-Learning)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
...
plt.title('Resulted Total Participants (Q-Learning)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
...
plt.title('Resulted Overall Losses (Q-Learning)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
...
plt.title('Resulted Number of JVBs (Q-Learning)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
print("Fifth Plotting time: %d seconds" % (time.time() - curr_time))
...
plt.title('Resulted Overall RTTs (Q-Learning)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
