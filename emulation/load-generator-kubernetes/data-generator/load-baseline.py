...
curr_time = time.time()
# Query InfluxDB
monitoring_data = influxdb_jcf.query("select conferences, participants, bridge_selector_bridge_count from jicofo_stats where time >= '2020-12-09T10:37:00Z' and time < '2020-12-09T14:15:00Z';")
monitoring_data2 = influxdb_jvb.query("select mean(jitter_aggregate) as jitter, mean(overall_loss) as loss, mean(rtt_aggregate) as rtt from jitsi_stats where time >= '2020-12-09T10:37:00Z' and time < '2020-12-09T14:15:00Z' group by time(1s) fill(0);")
...
plt.title('Resulted Total Conferences (Baseline)')
...
plt.title('Resulted Overall Jitters (Baseline)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
...
plt.title('Resulted Total Participants (Baseline)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
...
plt.title('Resulted Overall Losses (Baseline)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
...
plt.title('Resulted Number of JVBs (Baseline)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
print("Fifth Plotting time: %d seconds" % (time.time() - curr_time))
...
plt.title('Resulted Overall RTTs (Baseline)')
ax.xaxis.set_major_formatter(xformatter)
plt.grid(True)
